...
| 行 | 左侧 | 右侧 |
|----|------|------|
| **第一行** | Themis · 主题徽章 | 捕捉 · 诊断 · 中文 · **清空** |
| **第二行** | 透 −/+ · 字 −/+ ↺（透明度 / 字号） | 浮 · 尺寸 · 隐藏 · 退出 |
...
1. 打开 GitHub 仓库 **Releases** 页（标签形如 `v0.5.0` 的发布）。
2. 若尚无 Release，需维护者先 [触发 Release Pipeline](#3-触发-release-pipeline)；在此之前只能从 [源码构建](#4-本地编译与调试)。

| 你的系统 | 下载 |
|----------|------|
| **Windows 64 位** | `Themis-windows-x86_64.zip` → 解压后进入 `Themis-Windows/` |
| **macOS Apple Silicon** | `Themis-macos-aarch64.zip` → `Themis-macOS-Apple-Silicon/` |
| **macOS Intel** | `Themis-macos-x86_64.zip` → `Themis-macOS-Intel/` |

每个 ZIP 内已包含 **themis-tray**、**themis-service**、**README.md**、**.env.example** 等同目录文件，无需从二十多个平铺附件里挑选。

- **便携（推荐）**：解压后运行 `themis-tray` / `themis-tray.exe`
- **安装包（可选）**：Windows `*-setup.exe`；macOS `.dmg`（均在 ZIP 内）
...
2. 成功后 **Releases** 页会出现 `v0.1.0`，附件为 **3 个平台 ZIP** + `README.md`（如 `Themis-windows-x86_64.zip`、`Themis-macos-aarch64.zip` 等）
3. Release 说明正文来自 `packaging/RELEASE-INDEX.md`，并附加自动生成的 changelog
...
│    ③ package-release-assets → 平台 ZIP + upload-artifact │
└───────────────────────────┬─────────────────────────────┘
                            ▼
┌─────────────────────────────────────────────────────────┐
│  release (ubuntu-latest)                                 │
│    合并 artifacts → 上传 Themis-*.zip + README.md         │
└─────────────────────────────────────────────────────────┘
```
...
GitHub Actions 不可用（额度、网络等）或发 tag 前想在本地验包时，可在**对应平台**的项目根目录执行 **一条命令**，产出与 CI Release job 相同结构的 ZIP。
...
脚本会依次：前端 `npm ci` + 构建 → `cargo build --release`（service / cli / tray）→ Tauri 安装包（Windows NSIS / macOS dmg）→ `package-release-assets` 收集并打 ZIP。

**产出：** `release-assets/Themis-<platform>.zip`（解压后为 `Themis-Windows/` 等文件夹，内含 `themis-tray`、`themis-service`、`.env.example`、`README.md`）。本地中间目录：`release-assets/windows-x86_64/` 或 `release-assets/macos-aarch64/`。
